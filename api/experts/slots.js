// /api/experts/slots.js - VERSION FINALE PRO (Securisée & Filtrée)
...
module.exports = async (req, res) => {
    ...
    try {
        ...
        const now = new Date();
        const safeNow = new Date(now.getTime() + 5 * 60000); // Marge de 5 min
        ...
        // 2. REFRESH AUTOMATIQUE (Si nécessaire)
        // C'est transparent pour l'utilisateur : si son token Google est vieux mais sa session active, on le rafraichit.
        if (!access_token || (expiry_date && Date.now() > Number(expiry_date) - 60000)) {
            if (refresh_token && process.env.GOOGLE_CLIENT_ID && process.env.GOOGLE_CLIENT_SECRET) {
                ...
                if (refData && refData.access_token) {
                    ...
                    // Mise à jour silencieuse en base
        ...
        // 4. DECOUPAGE & FILTRAGE (Passé masqué)
        const SLOT_MIN = 30;
        const slots = [];
        ...
        for (const range of availRanges) {
            ...
            while (cursor.getTime() + SLOT_MIN * 60000 <= endMs) {
                ...
                // Si c'est dans le passé, on saute
                if (sStart < safeNow) {
